...
class HuffmanEncoder:
    ...
    def __init__(self, bitout):
        ...
        self.codetree = None  # why not as a parameter? this property may be changed. 
    ...
    def write(self, symbol):
        ...
        bits = self.codetree.get_code(symbol)  # the inputfile is read in byte mode. 
        size = 0
        for b in bits:
            size+=self.output.write(b)
        return size
...
class FrequencyTable:
    ...
    def build_code_tree(self):
        ...
        for (i, freq) in enumerate(self.frequencies):
            if freq > 0:								    # when comparing, compare the former elements one by one
                heapq.heappush(pqueue, (freq, i, Leaf(i)))  # heapq is a smalleset heap (a priortiy queue)
        ...
        while len(pqueue) > 1:         # remain 1 element in pqueue but is a root of InternalNode tree whose leveas include symbols
...
#          / \
#         C   D
class CodeTree:  # the code is tuple or None
    ...
    def __init__(self, root, symbollimit):
        ...
        self.codes = [None] * symbollimit   # symbol only support those types (can be used for list indexing) 
    ...
    def __str__(self):
        ...
        def to_str(prefix, node):
            if isinstance(node, InternalNode):
                ...
                return "Code {}: Symbol {}\n".format(prefix, node.symbol)  # why not print self.codes directly?
            else:
                raise AssertionError("Illegal node type")
...
# An internal node in a code tree. It has two nodes as children (lelf and right).
...
class CanonicalCode:
    ...
    def __init__(self, codelengths=None, tree=None, symbollimit=None):
        if codelengths is not None and tree is None and symbollimit is None:
            ...
            if symbollimit < 2:
                raise ValueError("At least 2 symbols needed")
            self.codelengths = [0] * symbollimit  # codelength: leaf depth in codetree
        ...
        for i in range(max(self.codelengths), -1, -1):  # Descend through code lengths
            ...
            if i > 0:
                for (j, codelen) in enumerate(self.codelengths):
                    if codelen == i:
                        newnodes.append(Leaf(j))    # newnodes append all i layer leaves 
            
            # Merge pairs of nodes from the previous deeper layer
            for j in range(0, len(nodes), 2):
                newnodes.append(InternalNode(nodes[j], nodes[j + 1]))
            nodes = newnodes       					# last, last and last layer leaves
        
        assert len(nodes) == 1
        return CodeTree(nodes[0], len(self.codelengths))  # why codelength, not frequency?
...
class BitInputStream:
    ...
    def read(self):
        if self.currentbyte == -1:
            return -1
        if self.numbitsremaining == 0:      # 读完了，重新取1 byte
            temp = self.input.read(1)
            if len(temp) == 0:				# 想要读取一个字节，但是读取不到，意味着文件末尾
                print('EOF')
                self.currentbyte = -1
                return -1
        ...
        self.numbitsremaining -= 1			# 7
        return (self.currentbyte >> self.numbitsremaining) & 1  # 位操作进行的是逻辑操作，还是算数操作
...
class BitOutputStream:
    ...
    def write(self, b):
        ...
        if self.numbitsfilled == 8:
            towrite = bytes((self.currentbyte,))  # every 8 bits (1 byte) write into the file
