...
class ArithmeticCoderBase:
    ...
    def __init__(self, numbits):
        ...
        # Maximum range (high+1-low) during coding (trivial), which is 2^num_state_bits = 1000...000.  (33bit)
    ...
    def update(self, freqs, symbol):
        ...
        symlow = freqs.get_low(symbol)		# 0
        symhigh = freqs.get_high(symbol)    # 1
        ...
        # Update range
        newlow = low + symlow * range // total       # 从图片的freq 缩放到 31bit+2上       011111...//786432 = 10 0*10 1*10-1
        newhigh = low + symhigh * range // total - 1 # why minus 1?
        ...
        # self.half_range  	  0100...000	& half: only get the second bit
        # 									^ half: change the second bit 1 to 0 or 0 to 1, the other bits remain the same.
        # 									| half: change the second bit to 1 whether it is 0 or 1 before, the other bits remain the same.
        # self.quarter_range  0010...000	& quarter: only get the third bit
        # self.state_mask     0111...111   	& mask: the first bit must be 0 ,to make sure the integer is positive in language like C++, the other bits remain the same.
        # | 1: make the last bit 1 (add 1 or not, | is faster than add)

        # first bit is sign bit?
        # While low and high have the same top bit value, shift them out
        # process the second bit indeed ==> 0 0...0 1...
        while ((self.low ^ self.high) & self.half_range) == 0:
            self.shift()
            self.low = ((self.low << 1) & self.state_mask)
            self.high = ((self.high << 1) & self.state_mask) | 1

        # After the former op, now low's top bit must be 0 and high's top bit must be 1
        # take attention to the third bit 1 or 0
        # While low's top two bits are 01 and high's are 10, delete the second highest bit of both
        # low: 0 0111110... ==>0 00... or 0 00.. or 0 01  (the next symbol code must in this range [low, high])
        while (self.low & ~self.high & self.quarter_range) != 0:
            self.underflow()														 # high:0 1000000... ==>0 10... or 0 11.. or 0 11
            # pop the first 2,3 bit one time
            self.low = (self.low << 1) ^ self.half_range
            self.high = ((self.high ^ self.half_range) <<
                         1) | self.half_range | 1   # 0 1...1
...
class ArithmeticEncoder(ArithmeticCoderBase):
    ...
    def shift(self):
        bit = self.low >> (self.num_state_bits - 1)  # 0 111110001...
        self.size += self.output.write(bit)				         # 0 111110010...

        # Write out the saved underflow bits
        for _ in range(self.num_underflow):          # 只需要在第一次进行处理即可。
            # 首位进来为1，说明了这个symbol映射到了上一次范围中的上半部分。为0，则是下半部分。
            self.size += self.output.write(bit ^ 1)
        self.num_underflow = 0						 # 进来0，则补上1（0 111 0（相当于遇见了这个0） 1111）
        # 进来1，则补上0（0 111 1（相当于遇见了这个1） 0000）
...
class ArithmeticDecoder(ArithmeticCoderBase):
    ...
    def __init__(self, numbits, bitin):
        ...
        for _ in range(self.num_state_bits):
            self.code = self.code << 1 | self.read_code_bit()   # 1 +32bit
    ...
    def read(self, freqs):
        ...
        value = ((offset + 1) * total - 1) // range  # 根据偏移，还原为原来的累积频数
    ...
    def shift(self):
        self.code = ((self.code << 1) &
                     self.state_mask) | self.read_code_bit()      # 不断补充新的位数

    # 看不懂的其实只是underflow
    def underflow(self):
        # underflow一次，其实也需要读取一位。code需要左移，将之前已经解码完成的信息丢弃。
        # 取第二位                                     去除前两位
        # 为什么要先去掉，再补上？仅仅往前挪动一位，会让本来的最高位，变成符号位。
...
class BitOutputStream:
    ...
    def write(self, b):
        ...
        self.currentbyte = (self.currentbyte << 1) | b  # add one bit
        self.numbitsfilled += 1
        if self.numbitsfilled == 8:
            # every 8 bits (1 byte) write into the file
    ...
    def close(self):
        while self.numbitsfilled != 0:  # code_len_table doesn't need this.
            self.write(0)  # padding would cal in write()
        print(f'totally: {self.size} bytes (plus padding)')
        self.output.close()
